...
const inscrire = async (donnees) => {
    const { nom, prenom, email, mot_de_passe, role, niveau_scolaire, matiere, titre, numero_telephone } = donnees;
    ...
    try {
        if (existant && !existant.est_verifie) {
            await modele.mettreAJour(existant.id, { nom, prenom, mot_de_passe: motDePasseHache, niveau_scolaire, matiere, titre, numero_telephone });
        } else {
            await modele.creer({ nom, prenom, email, mot_de_passe: motDePasseHache, niveau_scolaire, matiere, titre, numero_telephone });
        }
    } catch (err) {
...
const connecter = async ({ email, mot_de_passe, role }) => {
    ...
    if (utilisateur.est_verifie === false) throw new Error('Compte non verifie. Verifiez votre email.');
