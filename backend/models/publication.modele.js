...
const creer = async ({ titre, description, contenu, media_url, niveau_scolaire, prix, enseignant_id }) => {
    const resultat = await pool.query(
        'INSERT INTO publications (titre, description, contenu, media_url, niveau_scolaire, prix, enseignant_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
        [titre, description, contenu, media_url, niveau_scolaire, prix || 0, enseignant_id]
    );
...
const modifierParId = async (id, { titre, description, contenu, media_url, niveau_scolaire, prix }) => {
    const resultat = await pool.query(
        'UPDATE publications SET titre=$1, description=$2, contenu=$3, media_url=$4, niveau_scolaire=$5, prix=$6 WHERE id=$7 RETURNING *',
        [titre, description, contenu, media_url, niveau_scolaire, prix, id]
    );
    return resultat.rows[0];
};
